...
version = re.match(r'__version__ = "(.+)"', raw_version)[1]
...
first_supported_version = (
    f"{supported_python_versions[0][0]}.{supported_python_versions[0][1]}"
)

# next minor after the latest supported version
first_unsupported_version = f"{supported_python_versions[-1][0]}.{supported_python_versions[-1][1] + 1}"
...
setup(
    ...
    packages=find_packages(
        where=".", exclude=("tests*", "benchmarks*", "it*")
    ),
    include_package_data=True,
    python_requires=f">={first_supported_version},<{first_unsupported_version}",
